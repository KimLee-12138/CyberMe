"""Sync ASM course documents from Vault to database."""
...
DIRS = [
    "20_University/ASM_汇编/00_汇编_课程MOC.md",
    "20_University/ASM_汇编/02_知识点",
    "20_University/ASM_汇编/03_公式与规则",
    "20_University/ASM_汇编/04_例题与错题",
]
        ...
        for d in DIRS:
            ...
            for fp, rp in files:
                ...
                if row and row[1] != nh:
                    ...
                    await db.execute(text("INSERT INTO vault_documents (id, vault_id, relative_path, title, course_code, document_type, markdown_body, content_hash, status, created_at, updated_at) VALUES (gen_random_uuid(), (SELECT vault_id FROM vault_documents WHERE course_code=:c LIMIT 1), :p, :t, :c, :dt, :b, :h, 'active', NOW(), NOW())"), {"c": "ASM", "p": rp, "t": title, "dt": dt, "b": content, "h": nh, "active": "active"})
                    updated += 1
        await db.commit()
        print(f"ASM: {updated} docs updated/inserted. Total {total_chars} chars.")
