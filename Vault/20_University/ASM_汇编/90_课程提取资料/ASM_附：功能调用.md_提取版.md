---
id: extract-asm-0fff73b5
type: extract
status: extracted
course: ASM
source:
  - "[[91_Raw-Archive/DOC/ASM_附：功能调用.md_笔记_未知日期_0fff73b5.md]]"
source_pages: all
source_hash: "0fff73b5fd3a0dfe2a0258a9c64fba413b3a2139ff114323e0696adb29235e3a"
...
当你不需要打印一长串，只想在屏幕上输出一个字符（比如换行符，或者循环打印出来的单个数字）时，用这个最轻量级。
...
```
mov dl, 'A'    ; 把字符 'A' 放入 DL (也可以直接写 41H)
...
当你的程序需要暂停，等待用户敲击一下键盘（比如“按任意键继续”，或者输入一个选项 `Y/N`）时使用。

- **所需参数：** 无需提前准备参数。
- **执行结果：** DOS 会挂起程序，等你按键。按完后，这个字符会显示在屏幕上，并且它的 ASCII 码会被自动存入 **`AL`** 寄存器。
- **代码模板：**
...
int 21h        ; 呼叫 DOS 执行。此时屏幕光标闪烁，等待敲击
               ; 用户按下 'B' 后，AL 寄存器里就会变成 42H ('B'的ASCII码)
```
...
这是最复杂、但也最强大的一个功能。相当于 C 语言里的 `scanf`。它要求你必须提前在内存里建好一个**特定格式的缓冲区（Buffer）**。
...
data segment
    ; 构建缓冲区: 允许输入10个字符，实际长度预留0，后面留10个空位
