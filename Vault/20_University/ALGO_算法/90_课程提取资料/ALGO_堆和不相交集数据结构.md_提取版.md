---
id: extract-algo-017308fa
type: extract
status: extracted
course: ALGO
source:
  - "[[91_Raw-Archive/DOC/ALGO_堆和不相交集数据结构.md_笔记_未知日期_017308fa.md]]"
source_pages: all
source_hash: "017308fa0567e346c8798ab263736f242fc00d5cf070aaffabb95b0f72f143fe"
...
- **定义**：堆通常是一个**完全二叉树 **。
- **性质**：
  - **最大堆 **：任意节点的值都 $\ge$ 其子节点的值。根节点是最大值 1。
  - **最小堆 **：任意节点的值都 $\le$ 其子节点的值。根节点是最小值 2。
- **存储结构**：通常使用**数组**来实现。
  - 对于下标为 $i$ 的节点：
    - 左孩子下标：$2i$
    - 右孩子下标：$2i + 1$
...
- **Sift-up (上滤/上浮)**：
  - **场景**：通常用于**插入 (Insert)**。
  - **过程**：将新元素放到数组末尾，如果它违反了堆序性（例如比父节点大），就与父节点交换，直到满足性质。
  - **时间复杂度**：$O(\log n)$。
...
- **Sift-down (下滤/下沉)**：
  ...
  - **过程**：将根节点移走，把数组最后一个元素放到根节点位置，然后让它与较大的子节点交换，直到沉到合适的位置。
  - **时间复杂度**：$O(\log n)$。
...
- **Insert (插入)**：
  - 逻辑：将元素添加到末尾 $\rightarrow$ 执行 `Sift-up`。
...
- **Delete-max (删除最大值)**：
  - 逻辑：移除根节点 $\rightarrow$ 将末尾元素移至根部 $\rightarrow$ 执行 `Sift-down`。
...
- **方法**：
  ...
  1. **自顶向下 (Top-down)**：通过不断 `Insert` 实现，复杂度 $O(n \log n)$。

  伪代码：

  ```c++
  Function BuildHeap_TopDown(A, n):
  ...
  - **优势**：这种方法的总时间复杂度是 **$O(n)$**（线性时间），比插入法更快。
...
PPT 中给出了一个具体的排序例子：`A[1..10]={4,3,8,10,11,13,7,30,17,26}` 
...
- **定义**：不相交集是一种数据结构，它维护了一系列**互不相交**的子集合。每个集合选出一个“代表”（Representative），通常是树的根节点，用来标识这个集合。
...
确定元素 $x$ 属于哪个子集。这通常通过不断向上查找父节点，直到找到根节点为止。

- **逻辑**：如果 $x$ 不是根，就找 $parent[x]$，一直找到根。
- **伪代码（基础版）**：
...
- 如果树退化成一条链（像链表一样），查找的时间复杂度会变成 $O(n)$，效率很低。

##### (3) Union(x, y) —— 合并

将元素 $x$ 所在的集合和元素 $y$ 所在的集合合并为一个集合。
...
**解决树过高的问题**。在合并两棵树时，不要随意合并，而是总是**把矮树（秩小的树）挂在原本就高的树（秩大的树）下面**。

- 这样可以避免树的高度增加过快，保证树的高度维持在 $O(\log n)$。
...
**解决重复查询效率低的问题**。在执行 `Find(x)` 的过程中，既然我们已经费力找到了根节点，不如**顺手把路径上经过的所有节点直接指向根节点**。
...
- 单次操作的平均时间复杂度接近 **$O(1)$**。
- 更严谨的说法是 **$O(\alpha(n))$**，其中 $\alpha$ 是阿克曼函数（Ackermann function）的反函数。
- 这个函数增长极慢，对于宇宙中任何实际规模的 $n$（例如 $n < 10^{80}$），$\alpha(n) \le 4$。因此，可以认为它是常数时间。
